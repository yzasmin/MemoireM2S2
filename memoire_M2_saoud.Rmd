...
partir de 1 602 lignes brutes, sans doublon détecté par les trois clés d'unicité retenues. J'ai ensuite mené de bout en bout un
...
le modèle de données : une opération est un conteneur, auquel se rattachent notamment
son identification, ses indicateurs, son foncier, ses intervenants, ses versions, ses
tranches, son budget, son planning, sa commercialisation et sa trésorerie. Cette arborescence n'est pas une commodité d'affichage,
...
elle n'était pas documentée du tout, et c'est elle qui a occupé la suite du travail.
...
L'étape de transformation mérite qu'on s'y arrête, car c'est celle qui a demandé le plus de
travail et celle dont dépendait la validité de tout le reste. Mapper un champ n'est pas le
renommer. Une valeur venue de Grimmo n'entre dans SPO que si elle appartient à la
nomenclature que le nouveau système reconnaît : un libellé de poste budgétaire, un type de
tranche, une nature d'opération y sont des valeurs contrôlées, et non du texte libre. Le
travail a donc consisté, champ par champ, à établir la correspondance entre ce que l'ancien
système contenait et ce que le nouveau accepte.

Ces correspondances n'ont pas toutes la même origine, et la distinction a des conséquences
pratiques. Certaines proviennent de tables de paramétrage extraites de SPO : ce sont les
listes de valeurs que l'ERP tient lui-même, donc des références sûres, qu'il suffit
d'interroger. D'autres, en revanche, sont des fichiers de mapping que j'ai constitués, parce
qu'aucune table ne les fournissait : la mise en regard des libellés de l'ancien système avec
ceux du nouveau relevait alors d'un arbitrage, tranché avec les services concernés puis
consigné. La différence est importante au moment de rendre des comptes : dans le premier cas,
une divergence est une erreur de lecture ; dans le second, c'est une décision qu'il faut
pouvoir justifier.

Le choix d'Excel comme moteur de transformation, plutôt qu'un script, découle de l'arbitrage
exposé plus haut. La fonction RechercheX y joue exactement le rôle d'une jointure : elle va
chercher, dans la table de correspondance, la valeur cible associée à la valeur source. Sa
vertu, dans ce contexte, est d'être lisible par les personnes qui connaissent le métier et
non le code. Lorsqu'un poste budgétaire ressortait mal traduit, la responsable concernée
pouvait ouvrir la maquette, voir la formule et pointer la ligne fautive de la table de
correspondance. Un script aurait produit le même résultat, mais aurait déplacé la
vérification vers moi seule.
...
Rédiger cette note n'était pas prévu au départ, et je ne l'ai pas fait par goût de la
formalisation. Je l'ai fait parce que j'avais constaté que je redécouvrais deux fois la même
contrainte, à quelques jours d'intervalle, faute de l'avoir notée. C'est le premier livrable
écrit de mon année qui n'était pas un fichier de données, et c'est celui qui a le plus servi
aux autres : la reprise des tiers, exposée au chapitre 3, s'est appuyée directement dessus,
et n'a pas eu à refaire le travail de rétro-ingénierie. Cet épisode a modifié ma façon de
travailler pour le reste de l'année. Une contrainte technique découverte empiriquement n'a de
valeur, pour l'entreprise, que si elle est écrite quelque part ; sinon elle reste attachée à
la personne qui l'a trouvée, et disparaît avec elle.
...
les opérations et leurs tranches de travaux, la création des tranches commerciales étant
encore en cours à la fin du semestre. Une part conséquente du travail restait à accomplir : l'analyse et
