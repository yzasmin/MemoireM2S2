...
contrôle «~A\_DM\_BUDGET\_MONTANT\_GESTION\_LIVE~», désigné ci-après comme le fichier LIVE.
...
Le premier concerne le fichier «~06\_Informations Lots~». Malgré son nom, il ne contient
...
\item
  \textbf{Le taux moyen des crédits habitat en France}, série mensuelle de la Banque centrale
  européenne {[}4{]}, de 2015 à 2026.
\item
  \textbf{L'indicateur de confiance des ménages}, publié par Eurostat {[}5{]}, sur la même période.
\item
  \textbf{Un référentiel des 90 communes d'implantation} du groupe, comprenant la population et
  les coordonnées géographiques, obtenu par l'API publique \texttt{geo.api.gouv.fr} {[}6{]} et enrichi
...
commune.
D'autre part, l'effet du taux d'intérêt est fortement significatif : \textbf{une hausse d'un point
de taux réduit les réservations mensuelles d'environ 19~\%}, pour un coefficient de -0,216
associé à une p-valeur inférieure à 10\textsuperscript{-15}. La régression naïve, qui ignore la structure en
...
d'environ +19 points entre 2016 et 2023, suivi d'un plateau.
...
La page \textbf{Alertes marge}, présentée en Figure 3.7, classe les opérations par niveau
...
Les trois autres pages transposent les axes restants. La page \textbf{Rythme de vente} fait de
...
estimé. La page \textbf{Pilotage des prix} affiche le diagnostic du stock et la grille
d'ajustement recommandée sous un objectif d'accélération choisi au curseur. La page
\textbf{Qualité commerciale} reprend les signaux faibles, alerte de saisie comprise.
...
séparés. Le \textbf{cadrage} formule les questions de gestion avant de regarder les données et pose
les exigences non fonctionnelles. L'\textbf{exploration et la modélisation} viennent ensuite, chaque
question étant traitée par la méthode la plus simple qui y réponde. La \textbf{restitution} traduit
...
Le régime de preuve applicable ici est celui de la \textbf{validation statistique}, où la question
...
Trois difficultés relèvent enfin de la donnée elle-même. L'\textbf{absence d'historique} d'abord :
...
dynamique d'une dérive sur une image fixe. La \textbf{qualité de saisie} ensuite, dont le cas le
...
L'\textbf{impossibilité d'estimer certains paramètres en interne} enfin : l'élasticité de la demande
