...
moyen des crédits habitat est passé d'environ 1,1 % à un pic de 3,6 % fin
2023, avant de refluer
...
soit 9 Mo une fois chargées en mémoire), Spark est cinquante-huit fois
**plus lent** que pandas sur l'agrégation chronométrée, après 23,9
secondes de démarrage de session — les chronos varient d'une machine à
l'autre, l'ordre de grandeur et la conclusion non. Le passage à l'échelle
est une assurance pour des volumétries futures, pas un accélérateur ;
présenter Spark comme un gain sur moins de 10 Mo serait une erreur de diagnostic.
...
positives). Sur ce périmètre, plus d'un tiers des opérations (36,6 %) n'a
aucun dépassement engagé et la médiane de variation est quasi nulle, mais
28 % franchissent le seuil de dérive matérielle fixé à 2 % de la marge,
avec deux cas extrêmes au-delà de la moitié de la marge budgétée
(Figure 5).
...
le paramètre choisi par validation croisée) stabilise les coefficients et
en permet une lecture comparée (Figure 6), mais le pouvoir explicatif
reste modeste — un R² de 0,08 sur l'échantillon test pour les moindres
carrés ordinaires, que la régularisation n'améliore pas — et je l'assume comme un résultat honnête : la
...
erreur type sont signalés (Figure 10) — 103 sont dans le marché, 8
au-dessus, 16 en dessous à la date de l'export. (La plateforme, qui
ré-estime le modèle sur l'ensemble des transactions, aboutit à un
classement légèrement plus clément : 111/4/12.)
